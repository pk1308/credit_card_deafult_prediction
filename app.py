...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        with gr.Column():
            ...
            SEX =gr.inputs.Dropdown(choices=[("Male", 1), ("Female", 2)], label="Gender")
            EDUCATION = gr.inputs.Dropdown(choices=[("Graduate School", 1), ("University", 2), ("High School", 3), ("Others", 4)], label="Education")
            MARRIAGE = gr.inputs.Dropdown(choices=[("Married", 1), ("Single", 2), ("Others", 3)], label="Marriage")
            AGE = gr.inputs.Slider(minimum=20, maximum=100, label="Age in years")
            PAY_O = gr.inputs.Slider(minimum=-2, maximum=7, label="Repayment status in September 2005 (scale same as above)")
            PAY_1 = gr.inputs.Slider(minimum=-2, maximum=7, label="Repayment status in August 2005 (scale same as above)")
            PAY_2 = gr.inputs.Slider(minimum=-2, maximum=7, label="Repayment status in July 2005 (scale same as above)")
            PAY_3 = gr.inputs.Slider(minimum=-2, maximum=7, label="Repayment status in June 2005 (scale same as above)")
            PAY_4 = gr.inputs.Slider(minimum=-2, maximum=7, label="Repayment status in May 2005 (scale same as above)")
            PAY_5 = gr.inputs.Slider(minimum=-2, maximum=7,label="Repayment status in April 2005 (scale same as above)")
            PAY_6 = gr.inputs.Slider(minimum=-2, maximum=7,label="Repayment status in March 2005 (scale same as above)")
            BILL_AMT1 = gr.inputs.Slider(minimum=-35000, maximum=1000000, label="Amount of bill statement in September 2005 (NT dollar)")
            BILL_AMT2 = gr.inputs.Slider(minimum=-35000, maximum=1000000,  label="Amount of bill statement in August 2005 (NT dollar)")
            BILL_AMT3 = gr.inputs.Slider(minimum=-35000, maximum=1000000,label="Amount of bill statement in July 2005 (NT dollar)")
            BILL_AMT4 = gr.inputs.Slider(minimum=-35000, maximum=1000000,  label="Amount of bill statement in June 2005 (NT dollar)")
            BILL_AMT5 = gr.inputs.Slider(minimum=-35000, maximum=1000000,  label="Amount of bill statement in May 2005 (NT dollar)")
            BILL_AMT6 = gr.inputs.Slider(minimum=-35000, maximum=1000000, label="Amount of bill statement in April 2005 (NT dollar)")
